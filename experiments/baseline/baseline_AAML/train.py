...
from reidlib.utils.softmaxloss import ArcfaceLoss
...
def parm_list_with_Wdecay(model, losses):
...
def train_one_epoch(model, train_loader, losses, optimizer, scheduler, epoch):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        with amp.autocast():
            f_bn, p = model(imgs)
            ce_loss, p = losses['cross_entropy_loss'](f_bn, labels)
...
def prepare(args):
    ...
    losses = {}
    losses['cross_entropy_loss'] = ArcfaceLoss(Config.in_planes, Config.nr_class, Config.am_scale, Config.am_margin)
    losses['triplet_hard_loss'] = triplet_hard_loss(
        margin=Config.triplet_margin)

    weight_decay_setting = parm_list_with_Wdecay(model, losses)
