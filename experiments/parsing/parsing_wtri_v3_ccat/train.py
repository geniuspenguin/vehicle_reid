...
from reidlib.utils.loss import triplet_hard_loss, weighted_triplet_batch_all_loss, thres_cross_entropy
...
def test(model, branches, test_loader, losses, epoch, nr_query=Config.nr_query):
    ...
    for branch in branches:
        branch.eval()
    for i, (imgs, labels, cids, masks) in tqdm(enumerate(test_loader), desc='testing on epoch-{}'.format(epoch), total=len(test_loader)):
        imgs, labels, cids, masks = imgs.cuda(), labels.cuda(), cids.cuda(), masks.cuda()
        x = model(imgs)
        fts = []
        for i, branch in enumerate(branches):
            if i == 0:
                ...
                f = branch(x, mask)
            fts.append(f)
        feat_signature = torch.cat(fts, axis=1)
        triplet_hard_loss = losses['triplet_hard_loss'][0](feat_signature, labels)
        history['triplet_hard_loss'].append(float(triplet_hard_loss))
        all_features.append(feat_signature.cpu().detach().numpy())
...
def prepare(args):
    ...
    testset = Veri776_test(transforms=test_transforms, need_mask=True)
    ...
    if resume_from_checkpoint and os.path.exists(Config.checkpoint_path):
        ...
        for i, branch in enumerate(branches):
            branch.load_state_dict(checkpoint['branch_{}'.format(i)])
    ...
    for epoch in range(start_epoch, Config.epoch + 1):
        ...
        if epoch % Config.epoch_per_save == 0:
            ...
            save_dict = {'model': model.state_dict(),
                         ...
                         'mAP': mAP}
            for i, branch in enumerate(branches):
                save_dict.update({'branch_{}'.format(i): branch.state_dict()})
