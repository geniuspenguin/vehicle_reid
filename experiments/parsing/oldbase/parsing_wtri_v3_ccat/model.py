...
class parsing_branch(nn.Module):
    def __init__(self, nr_class, in_planes):
        ...
        self.bn = nn.BatchNorm1d(self.in_planes)
        self.fc_512 = nn.Linear(self.in_planes, 512)
        self.bn_512 = nn.BatchNorm1d(512)
        self.fc_cls = nn.Linear(512, self.nr_class, bias=False)

        self.fc_512.apply(weights_init_kaiming)
        self.bn_512.apply(weights_init_kaiming)
        self.fc_cls.apply(weights_init_classifier)
        self.bn.apply(weights_init_kaiming)
    ...
    def forward(self, x, mask):
        ...
        feats = self.fc_512(mapx)
        feats = self.bn_512(feats)
